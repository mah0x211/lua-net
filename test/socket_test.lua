...
function testcase.new_inet_opts_all()
    -- Every stream-level bool / int / timeval opt recognized by
    -- new_inet_specs applies without error, driving all cfg_check_* and
    -- sockopts_apply branches for stream sockets.
    local s = assert(socket.new_inet({
        socktype = 'stream',
        protocol = 'tcp',
        debug = false,
    ...
    -- dgram + multicast opts (broadcast/mcastloop/mcastttl/mcastif) on IPv4.
...
function testcase.new_inet_setsockopt_failures()
    -- Out-of-range sndbuf / rcvbuf makes sockopts_apply()'s setsockopt
    -- fail, so new_inet takes the "close and return NULL" cleanup branch.
    ...
    -- Drive each per-option setsockopt failure branch of sockopts_apply.
...
function testcase.recvmsg_oom()
    -- pass a bufsize / cmsgbuf so large that malloc() fails, driving the
    -- recvmsg_lua ENOMEM branches (databuf == NULL and controlbuf == NULL).
    local socks = assert(socket.pair({
        socktype = 'stream',
    }))
    local a = socks[1]
