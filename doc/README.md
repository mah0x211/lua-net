# lua-net module API Reference


## Constants

- [Constants in net module](constants.md)

## Libraries

- [net.addrinfo](addrinfo.md)
- [net.cmsghdr](cmsghdr.md)
- [net.device](device.md)
- [net.socket](socket.md)

## Classes

- [net.MsgHdr](msghdr.md)
- [net.Socket](net_socket.md)
- [net.unix.Socket](net_unix_socket.md)
- [net.stream.Socket](net_stream_socket.md)
    - [net.stream.inet.Socket](net_stream_inet_socket.md)
        - [net.stream.inet.Client](net_stream_inet_client.md)
    - [net.stream.unix.Socket](net_stream_unix_socket.md)
        - [net.stream.unix.Client](net_stream_unix_client.md)
    - [net.stream.Server](net_stream_server.md)
        - [net.stream.inet.Server](net_stream_inet_server.md)
        - [net.stream.unix.Server](net_stream_unix_server.md)
- [net.dgram.Socket](net_dgram_socket.md)
    - [net.dgram.inet.Socket](net_dgram_inet_socket.md)
    - [net.dgram.unix.Socket](net_dgram_unix_socket.md)

